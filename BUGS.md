...
- **Estado**: 🔵 Aberto
- `since_id=abc` resulta em erro 500 não tratado. `limit=-1` remove o limite no SQLite. `limit` no MCP não possui teto máximo.

### [QUAL-002] Passwords expostas em query string e hashing com 1 iteração
- **Estado**: 🔵 Aberto
- Passwords em query string ficam registadas em histórico do browser e logs de servidor.

### [QUAL-003] Falta do campo `poll.votes` na Web UI
- **Estado**: 🔵 Aberto
- `index.html:902` procura por `poll.votes`, que não é retornado pelo backend, impedindo o destaque do voto do utilizador.
